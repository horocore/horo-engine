...
namespace Horo::Runtime::Ui::RenderGeometryInternal {
    ...
    void AppendQuad(std::vector<UiRenderVertex> &vertices, std::vector<std::uint32_t> &indices, const QuadDescriptor &quad) {
        ...
        const auto transformed = std::array<FloatPoint, 4>{
    ...
    void AppendNineSlice(std::vector<UiRenderVertex> &vertices, std::vector<std::uint32_t> &indices, const UiLogicalTransform &transform,
        ...
        const auto x = static_cast<float>(rect.origin.x);
        const auto y = static_cast<float>(rect.origin.y);
        const auto width = static_cast<float>(rect.extent.width);
        const auto height = static_cast<float>(rect.extent.height);
        const auto sourceWidth = static_cast<float>(draw.sourceExtent.width);
        const auto sourceHeight = static_cast<float>(draw.sourceExtent.height);
        const float uvWidth = draw.uv[2] - draw.uv[0];
        const float uvHeight = draw.uv[3] - draw.uv[1];
        const auto leftFraction = static_cast<float>(draw.insets.left) / sourceWidth;
        const auto rightFraction = static_cast<float>(draw.insets.right) / sourceWidth;
        const auto topFraction = static_cast<float>(draw.insets.top) / sourceHeight;
        const auto bottomFraction = static_cast<float>(draw.insets.bottom) / sourceHeight;
        const auto leftInset = static_cast<float>(draw.insets.left);
        const auto rightInset = static_cast<float>(draw.insets.right);
        const auto topInset = static_cast<float>(draw.insets.top);
        const auto bottomInset = static_cast<float>(draw.insets.bottom);
