...
namespace Horo::Extensions {
    ...
    namespace {
        ...
        constexpr std::size_t MaximumOperationTypes = 64;
        constexpr std::size_t MaximumDiagnostics = 4096;
        constexpr std::size_t MaximumDiagnosticBytes = 1U << 20U;
        constexpr std::size_t MaximumResultBytes = 64U << 20U;

        [[nodiscard]] bool ValidIdentity(const std::string_view value) noexcept {
            return Detail::IsCanonicalExtensionAuthorityId(value);
        }

        [[nodiscard]] bool ValidTypedId(const std::string_view value) noexcept {
            return Detail::IsCanonicalExtensionAuthorityId(value) && value.size() <= MaximumIdentityBytes;
        }

        [[nodiscard]] bool ValidProvider(const ApplicationCapabilityProviderIdentity &provider) noexcept {
            return ValidIdentity(provider.moduleId) && ValidIdentity(provider.providerId) && provider.generation != 0;
        }

        [[nodiscard]] bool ValidProviderDescriptor(const BackendOperationProviderDescriptor &descriptor) noexcept {
        ...
        [[nodiscard]] bool ValidOperationDescriptor(const BackendOperationDescriptor &descriptor) noexcept {
            return ValidTypedId(descriptor.initialPhase.value);
        }

        [[nodiscard]] bool ValidProgress(const BackendOperationProgress progress) noexcept {
        ...
        [[nodiscard]] bool IsProgressRegression(const BackendOperationProgress current, const BackendOperationProgress next) noexcept {
        ...
        [[nodiscard]] Error CancellationError() {
            return MakeError(ExtensionErrors::BackendOperationCancelled);
        }

        [[nodiscard]] Error AbandonmentError() {
