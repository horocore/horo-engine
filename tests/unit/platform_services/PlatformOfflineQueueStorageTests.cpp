...
namespace {
    ...
    struct TemporaryRoot final {
        TemporaryRoot() {
            ...
            root = std::filesystem::temp_directory_path() / ("horo-offline-queue-" + std::to_string(stamp));
    ...
    [[nodiscard]] PlatformOfflineQueueRecord Record(const std::uint8_t identity, const std::uint8_t partition, const std::uint64_t sequence,
                                                    const std::string_view payload) {
        return PlatformOfflineQueueRecord{.identity = Intent(identity),
                                          .partition = Partition(partition),
                                          .state = PlatformOfflineIntentState::Pending,
                                          .operation = PlatformOfflineOperationKind::ProgressionMutation,
                                          .sequence = sequence,
    ...
    TEST_CASE("Offline queue storage round-trips bounded opaque records atomically", "[platform-services][offline][storage]") {
    ...
    TEST_CASE("Offline queue storage rejects malformed integrity version and bounds", "[platform-services][offline][storage][validation]") {
        TemporaryRoot temporary;
        Horo::NativeDurableFileSystem files;
