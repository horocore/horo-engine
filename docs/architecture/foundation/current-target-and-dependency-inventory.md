...
| `HoroEngine::NetworkTransportGNS` | Optional (`HORO_BUILD_NETWORK_GNS=ON`) | ADR-097 production direct-IP GNS transport behind `NetworkApi`. Explicit factory resides in a narrow, non-installed host/backend internal interface; native headers stay private. The default OFF profile neither fetches nor links GNS/protobuf. The ON profile pins GNS v1.6.0 and protobuf v3.21.12 to exact commits, builds protoc locally, and selects OpenSSL on Linux/macOS or BCrypt on Windows. GNS and protobuf are BSD-3-Clause licensed and binary redistributions need their license notices. This target does not automatically compose or select a backend. |
