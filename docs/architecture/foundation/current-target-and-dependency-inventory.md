...
| `HoroRuntimeUi` (`HoroEngine::RuntimeUi`) | Always | Owns backend-neutral identities, authored/cooked document contracts, canvas modes and deterministic logical-to-screen resolution, typed viewport evidence and safe-area/UI/font-scale/pixel-snap policy resolution, canvas references, mutable instance lifecycle, bounded retained element trees, deferred structural transactions, typed style assets/tokens/classes, deterministic element inheritance and visual-state resolution, incremental measure-arrange caching, immutable style/layout/render snapshots, per-view render extraction, dependency enumeration, and typed stale-value rejection under `Runtime/Ui/**`. Platform owns native evidence publication and Renderer owns native realization; Runtime UI does not query either. | Foundation, Assets (public) |
...
| Real target (alias) | Availability | Owner and public/private boundary | Direct first-party dependencies |
|---|---|---|---|
| `HoroPhysicsModel` (`HoroEngine::PhysicsModel`) | Always | Owns inert backend-neutral Physics authoring identities, errors, and versioned scene component values. It contains no solver, runtime world, backend selection, or activation behavior. | Foundation, Assets, Runtime (public) |
| `HoroRuntimeScene` (`HoroEngine::RuntimeScene`) | Always | Owns runtime scene definition, scene state, saved-scene baseline admission, and scene component contracts under `Runtime/Scene/**`, excluding primitive mesh/catalog and Physics-owned authoring headers. | Foundation, Runtime (including Runtime/Save), RuntimeUi, GameplayApi, Assets, PhysicsModel, SceneModel (public) |
...
| `HoroTerrainRuntime` (`HoroEngine::TerrainRuntime`) | Always | Owns the bounded owner-lane async cook/load/edit preparation coordinator and exact-fence publication gate under `Terrain/TerrainAsyncJobs.h`. It uses the host-injected Foundation scheduler and owns no native provider or World Streaming cell authority. | TerrainApi, Foundation (public) |
...
| `HoroEngine::RuntimeUi` | Partial | `HoroRuntimeUi` owns typed identities plus distinct authored, cooked, scene-reference, mutable-instance, bounded retained-tree, atomic structural-command, typed style registry/resolution, incremental measure-arrange, immutable style/layout, and immutable render-extraction contracts. Service composition, concrete layout algorithms, and interaction routing remain planned. |
...
| `HoroEngine::NetworkApi` | Partial | Foundation-only target owns canonical bounded IPv4/IPv6/DNS endpoints, generation-checked listener/connection/peer handles, channel identity, stable protocol/message/schema/RPC/feature/close-reason identities, authority-epoch-scoped network-object identity, bounded immutable protocol/codec/replication/RPC registries, canonical bounded message framing and declaration fingerprints, exact transport delivery capability negotiation/admission, prepared packet-buffer/queue storage primitives, and the backend-neutral packet transport/event/stats interface. Sessions remain planned. |
...
| `HoroEngine::NetworkTransportGNS` | Optional (`HORO_BUILD_NETWORK_GNS=ON`) | ADR-097 production direct-IP GNS transport behind `NetworkApi`. Explicit factory resides in a narrow, non-installed host/backend internal interface; native headers stay private. The default OFF profile neither fetches nor links GNS/protobuf/c-ares. The ON profile pins GNS v1.6.0, protobuf v3.21.12 and c-ares v1.34.8 to exact commits, builds protoc locally, uses owner-polled cancellable c-ares DNS, and selects OpenSSL on Linux/macOS or BCrypt on Windows. GNS/protobuf are BSD-3-Clause and c-ares is MIT licensed; binary redistributions need their license notices. This target does not automatically compose or select a backend. |
